  ...
  class ShipNav
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SpaceRubyTraderClient::ShipNav` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SpaceRubyTraderClient::ShipNav`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flight_mode')
        self.flight_mode = attributes[:'flight_mode']
      else
    ...
    def list_invalid_properties
      ...
      if @flight_mode.nil?
        invalid_properties.push('invalid value for "flight_mode", flight_mode cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @flight_mode.nil?
      true
    end
    ...
    def hash
      [system_symbol, waypoint_symbol, route, status, flight_mode].hash
    end
