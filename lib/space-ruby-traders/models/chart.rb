  ...
  class Chart
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SpaceRubyTraderClient::Chart` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SpaceRubyTraderClient::Chart`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'submitted_by')
        self.submitted_by = attributes[:'submitted_by']
      end

      if attributes.key?(:'submitted_on')
        self.submitted_on = attributes[:'submitted_on']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@waypoint_symbol.nil? && @waypoint_symbol.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@waypoint_symbol.nil? && @waypoint_symbol.to_s.length < 1
      true
    end
    ...
    def hash
      [waypoint_symbol, submitted_by, submitted_on].hash
    end
