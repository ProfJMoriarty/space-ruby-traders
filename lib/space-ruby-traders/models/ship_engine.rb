  ...
  class ShipEngine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SpaceRubyTraderClient::ShipEngine` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SpaceRubyTraderClient::ShipEngine`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @speed < 1
        invalid_properties.push('invalid value for "speed", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      symbol_validator = EnumAttributeValidator.new('String', ["ENGINE_IMPULSE_DRIVE_I", "ENGINE_ION_DRIVE_I", "ENGINE_ION_DRIVE_II", "ENGINE_HYPER_DRIVE_I"])
    ...
    def symbol=(symbol)
      validator = EnumAttributeValidator.new('String', ["ENGINE_IMPULSE_DRIVE_I", "ENGINE_ION_DRIVE_I", "ENGINE_ION_DRIVE_II", "ENGINE_HYPER_DRIVE_I"])
    ...
    def speed=(speed)
      ...
      if speed < 1
        fail ArgumentError, 'invalid value for "speed", must be greater than or equal to 1.'
      end
    ...
    def hash
      [symbol, name, description, condition, integrity, speed, requirements].hash
    end
