  ...
  class ShipReactor
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SpaceRubyTraderClient::ShipReactor` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SpaceRubyTraderClient::ShipReactor`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'power_output')
        self.power_output = attributes[:'power_output']
      else
        self.power_output = nil
      end
    ...
    def list_invalid_properties
      ...
      if @power_output.nil?
        invalid_properties.push('invalid value for "power_output", power_output cannot be nil.')
      end

      if @power_output < 1
        invalid_properties.push('invalid value for "power_output", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      symbol_validator = EnumAttributeValidator.new('String', ["REACTOR_SOLAR_I", "REACTOR_FUSION_I", "REACTOR_FISSION_I", "REACTOR_CHEMICAL_I", "REACTOR_ANTIMATTER_I"])
      ...
      return false if @power_output.nil?
      return false if @power_output < 1
    ...
    def symbol=(symbol)
      validator = EnumAttributeValidator.new('String', ["REACTOR_SOLAR_I", "REACTOR_FUSION_I", "REACTOR_FISSION_I", "REACTOR_CHEMICAL_I", "REACTOR_ANTIMATTER_I"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] power_output Value to be assigned
    def power_output=(power_output)
      if power_output.nil?
        fail ArgumentError, 'power_output cannot be nil'
      end

      if power_output < 1
        fail ArgumentError, 'invalid value for "power_output", must be greater than or equal to 1.'
      end
    ...
    def hash
      [symbol, name, description, condition, integrity, power_output, requirements].hash
    end
